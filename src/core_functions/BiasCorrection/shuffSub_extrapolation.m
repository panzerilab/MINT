function [corrected_v, plugin_v] = shuffSub_extrapolation(inputs, outputs, corr, corefunc, varargin)
...
if strcmp(func2str(corefunc), 'MI') || strcmp(func2str(corefunc), 'TE')  || strcmp(func2str(corefunc), 'cTE')  || strcmp(func2str(corefunc), 'cMI')
    ...
    plugin_shuff = repmat({zeros(opts.shuff, nTimepoints)},1, length(outputs));
    for sIdx = 1:opts.shuff
        ...
        for outIdx = 1:length(outputs)
            plugin_shuff{outIdx}(sIdx,:) = shuff_v{outIdx};
        end
    end
    for i=1:opts.xtrp
        ...
        for outIdx = 1:length(outputs)
            for t = 1:nTimepoints
                if strcmp(corr,'qe_shuffSub')
                    ...
                    for shuffIdx = 1:opts.shuff
                        y = [plugin_shuff{outIdx}(shuffIdx,t),  shuff2{outIdx}(shuffIdx,t),  shuff4{outIdx}(shuffIdx,t)];
...
elseif strcmp(func2str(corefunc), 'PID')
    ...
    if opts.pid_constrained
        I_plugin.I1  = cell2mat(MI({inputs{1}, inputs{end}}, {'I(A;B)'}, plugin_opts));
        I_plugin.I2  = cell2mat(MI({inputs{2}, inputs{end}}, {'I(A;B)'}, plugin_opts));
        I_plugin.I12 = cell2mat(MI({cat(1, inputs{1}, inputs{2}), inputs{end}}, {'I(A;B)'}, plugin_opts));
    ...
    for sIdx = 1:opts.shuff
        ...
        if opts.pid_constrained
            I_plugin.I1sh  = cell2mat(MI({inputs_sh{1}, inputs_sh{end}}, {'I(A;B)'}, plugin_opts));
            I_plugin.I2sh  = cell2mat(MI({inputs_sh{2}, inputs_sh{end}}, {'I(A;B)'}, plugin_opts));
            I_plugin.I12sh = cell2mat(MI({cat(1, inputs_sh{1}, inputs_sh{2}), inputs_sh{end}}, {'I(A;B)'}, plugin_opts));
        end
    end
    for i=1:opts.xtrp
        randidx = randperm(nTrials, nTrials);
        ...
        for np=2:length(npartition)
            for pidx = 1:npartition(np)
                ...
                if opts.pid_constrained
                    if npartition(np)==2
                        I_part.I1_2  = cell2mat(MI({inputs_p{1}, inputs_p{end}}, {'I(A;B)'}, plugin_opts));
                        I_part.I2_2  = cell2mat(MI({inputs_p{2}, inputs_p{end}}, {'I(A;B)'}, plugin_opts));
                        I_part.I12_2 = cell2mat(MI({cat(1, inputs_p{1}, inputs_p{2}), inputs_p{end}}, {'I(A;B)'}, plugin_opts));
                    elseif npartition(np)==4
                        I_part.I1_4  = cell2mat(MI({inputs_p{1}, inputs_p{end}}, {'I(A;B)'}, plugin_opts));
                        I_part.I2_4  = cell2mat(MI({inputs_p{2}, inputs_p{end}}, {'I(A;B)'}, plugin_opts));
                        I_part.I12_4 = cell2mat(MI({cat(1, inputs_p{1}, inputs_p{2}), inputs_p{end}}, {'I(A;B)'}, plugin_opts));
                    end
                end
                % shuffle
                for shuffIdx = 1:opts.shuff
                    ...
                    if opts.pid_constrained
                        if npartition(np)==2
                            I_part.I1_2_sh(shuffIdx,:)  = I_part.I1_2_sh(shuffIdx,:)  + cell2mat(MI({inputs_sh{1}, inputs_sh{end}}, {'I(A;B)'}, plugin_opts))/2;
                            I_part.I2_2_sh(shuffIdx,:)  = I_part.I2_2_sh(shuffIdx,:)  + cell2mat(MI({inputs_sh{2}, inputs_sh{end}}, {'I(A;B)'}, plugin_opts))/2;
                            I_part.I12_2_sh(shuffIdx,:) = I_part.I12_2_sh(shuffIdx,:) + cell2mat(MI({cat(1, inputs_sh{1}, inputs_sh{2}), inputs_sh{end}}, {'I(A;B)'}, plugin_opts))/2;
                        elseif npartition(np)==4
                            I_part.I1_4_sh(shuffIdx,:)  = I_part.I1_4_sh(shuffIdx,:)  + cell2mat(MI({inputs_sh{1}, inputs_sh{end}}, {'I(A;B)'}, plugin_opts))/4;
                            I_part.I2_4_sh(shuffIdx,:)  = I_part.I2_4_sh(shuffIdx,:)  + cell2mat(MI({inputs_sh{2}, inputs_sh{end}}, {'I(A;B)'}, plugin_opts))/4;
                            I_part.I12_4_sh(shuffIdx,:) = I_part.I12_4_sh(shuffIdx,:) + cell2mat(MI({cat(1, inputs_sh{1}, inputs_sh{2}), inputs_sh{end}}, {'I(A;B)'}, plugin_opts))/4;
                        end
                    end
                end
            end
        ...
        if opts.pid_constrained
            if strcmp(opts.bias,'qe_shuffSub')
                for t = 1:nTimepoints
                    y = [I_plugin.I1(t), I_part.I1_2(t), I_part.I1_4(t)];
                    p = polyfit(x_extrap, y, 2);
                    I_corrected.I1QE(t) = I_corrected.I1QE(t) + p(3)/xtrp;
                    y = [I_plugin.I2(t), I_part.I2_2(t), I_part.I2_4(t)];
                    p = polyfit(x_extrap, y, 2);
                    I_corrected.I2QE(t) = I_corrected.I2QE(t) + p(3)/xtrp;
                    y = [I_plugin.I12(t), I_part.I12_2(t), I_part.I12_4(t)];
                    p = polyfit(x_extrap, y, 2);
                    I_corrected.I12QE(t) = I_corrected.I12QE(t) + p(3)/xtrp;
                    for shuffIdx = 1:opts.shuff
                        y = [I_plugin.I1sh(t), I_part.I1_2_sh(t), I_part.I1_4_sh(t)];
                        p = polyfit(x_extrap, y, 2);
                        I_corrected.I1Sh(shuffIdx,t) =  I_corrected.I1Sh(shuffIdx,t) + p(3)/xtrp;
                        y = [I_plugin.I2sh(t), I_part.I2_2_sh(t), I_part.I2_4_sh(t)];
                        p = polyfit(x_extrap, y, 2);
                        I_corrected.I2Sh(shuffIdx,t) = I_corrected.I2Sh(shuffIdx,t) + p(3)/xtrp;
                        y = [I_plugin.I12sh(t), I_part.I12_2_sh(t), I_part.I12_4_sh(t)];
                        p = polyfit(x_extrap, y, 2);
    ...
    if opts.pid_constrained
        ...
        switch opts.chosen_atom
            case'Red'
                ...
                syn_plugin  = I_plugin.I12-I_plugin.I1-I_plugin.I2+red_plugin;
                unq1_plugin  = I_plugin.I1-red_plugin;
                unq2_plugin  = I_plugin.I2-red_plugin;
            case 'Unq1'
                ...
                red_plugin = I_plugin.I1-unq1_plugin;
                syn_plugin =  I_plugin.I12-I_plugin.I2-unq1_plugin;
                unq2_plugin = I_plugin.I2-I_plugin.I1+unq1_plugin;
            case 'Unq2'
                ...
                red_plugin = I_plugin.I2-unq2_plugin;
                syn_plugin =  I_plugin.I12-I_plugin.I1-unq2_plugin;
                unq1_plugin = I_plugin.I1-I_plugin.I2+unq2_plugin;
            case 'Syn'
                syn = corrected_v{pos};
                ...
                red_plugin  = I_plugin.I1+I_plugin.I2-I_plugin.I12+syn_plugin;
                unq1_plugin  = I_plugin.I12-I_plugin.I2-syn_plugin;
                unq2_plugin  = I_plugin.I12-I_plugin.I1-syn_plugin;
        end
...
elseif strcmp(func2str(corefunc), 'cFIT') || strcmp(func2str(corefunc), 'FIT')
    ...
    for sIdx = 1:opts.shuff
        ...
        for outIdx = 1:length(outputs)   
            for idx = 1:length(atom1_plugin)
                pluginSh.atom1{outIdx}(idx,sIdx) = atom1_shuff{outIdx}(idx);
                pluginSh.atom2{outIdx}(idx,sIdx) = atom2_shuff{outIdx}(idx);
            end 
        end
    end
    for i=1:opts.xtrp
        ...
        for np=2:length(npartition)
            for pidx = 1:npartition(np)
                inputs_p = partition(inputs_s, npartition(np), pidx, 0);
                [~,~,~,atom1_plugin, atom2_plugin] = feval(corefunc, inputs_p, outputs, plugin_opts);
                for idx = 1:length(atom1_plugin)
                    for outIdx = 1:length(outputs)
                        if npartition(np)==2
                            part2.atom1{outIdx}(idx) = part2.atom1{outIdx}(idx) + atom1_plugin{outIdx}(idx)/2;
                            part2.atom2{outIdx}(idx) = part2.atom2{outIdx}(idx) + atom2_plugin{outIdx}(idx)/2;
                        elseif npartition(np)==4
                            part4.atom1{outIdx}(idx) = part4.atom1{outIdx}(idx) + atom1_plugin{outIdx}(idx)/4;
                            part4.atom2{outIdx}(idx) = part4.atom2{outIdx}(idx) + atom2_plugin{outIdx}(idx)/4;
                        end
                    end
                end
                % shuffle
        ...
        for outIdx = 1:length(outputs)
            if strcmp(corr,'qe_shuffSub')
                for idx = 1:length(atom1_plugin)
                    p = polyfit(x_extrap, [atom1_plugin{outIdx}(idx), part2.atom1{outIdx}(idx), part4.atom1{outIdx}(idx)], 2);
            ...
            elseif strcmp(corr,'le_shuffSub')
                for idx = 1:length(atom1_plugin)
                    p = polyfit(x_extrap(1:2), [atom1_plugin{outIdx}(idx), part2.atom1{outIdx}(idx)], 1);
                    correctedQE.atom1{outIdx}(idx) = correctedQE.atom1{outIdx}(idx) +(p(2)/xtrp);
                    p = polyfit(x_extrap(1:2), [atom2_plugin{outIdx}(idx), part2.atom2{outIdx}(idx)], 1);
                    correctedQE.atom2{outIdx}(idx) = correctedQE.atom2{outIdx}(idx) +(p(2)/xtrp);
                    for shuffIdx = 1:opts.shuff
                        y = [pluginSh.atom1{outIdx}(idx,shuffIdx), shuff2.atom1{outIdx}(idx,shuffIdx)];
                        p = polyfit(x_extrap(1:2), y, 1);
                        correctedSh.atom1{outIdx}(idx,shuffIdx)  =  correctedSh.atom1{outIdx}(idx,shuffIdx) + (p(2)/xtrp);
                        y = [pluginSh.atom2{outIdx}(idx,shuffIdx), shuff2.atom2{outIdx}(idx,shuffIdx)];
                        p = polyfit(x_extrap(1:2), y, 1);
        ...
        if strcmp(func2str(corefunc), 'FIT')
            for outIdx = 1:length(outputs)
                ...
                plugin_v_all{outIdx} = min(atom1_plugin{outIdx},atom2_plugin{outIdx});
            end
        elseif strcmp(func2str(corefunc), 'cFIT') 
            for outIdx = 1:length(outputs)
                ...
                atom1_n = min(atom1_plugin{outIdx});
                atom2_n = min(atom2_plugin{outIdx});
                corrected_v{outIdx} = atom1-atom2; 
                plugin_v_all{outIdx} = atom1_n-atom2_n; 
            end
        end 
    end
    plugin_v = plugin_v_all;
elseif strcmp(func2str(corefunc), 'II')
    [~,~,~,atom1_plugin, atom2_plugin] = feval(corefunc, inputs, outputs, plugin_opts);
    pluginSh.atom1 = repmat({zeros(opts.shuff,nTimepoints)},1, length(outputs));
    pluginSh.atom2 = repmat({zeros(opts.shuff,nTimepoints)},1, length(outputs));
    ...
    for i=1:opts.xtrp
        ...
        x_extrap = npartition./nTrials;
         for outIdx = 1:length(outputs)
            if strcmp(corr,'qe_shuffSub')
                for t = 1:nTimepoints
                    p = polyfit(x_extrap, [atom1_plugin{outIdx}(t), part2.atom1{outIdx}(t), part4.atom1{outIdx}(t)], 2);
            ...
            elseif strcmp(corr,'le_shuffSub')
                for t = 1:nTimepoints
                    p = polyfit(x_extrap(1:2), [atom1_plugin{outIdx}(t), part2.atom1{outIdx}(t)], 1);
                    correctedQE.atom1{outIdx}(t) = correctedQE.atom1{outIdx}(t) +(p(2)/xtrp);
                    p = polyfit(x_extrap(1:2), [atom2_plugin{outIdx}(t), part2.atom2{outIdx}(t)], 1);
                    correctedQE.atom2{outIdx}(t) = correctedQE.atom2{outIdx}(t) +(p(2)/xtrp);
                    for shuffIdx = 1:opts.shuff
                        y = [pluginSh.atom1{outIdx}(shuffIdx,t), shuff2.atom1{outIdx}(shuffIdx,t)];
                        p = polyfit(x_extrap(1:2), y, 1);
                        correctedSh.atom1{outIdx}(shuffIdx,t)  =  correctedSh.atom1{outIdx}(shuffIdx,t) + (p(2)/xtrp);
                        y = [pluginSh.atom2{outIdx}(shuffIdx,t), shuff2.atom2{outIdx}(shuffIdx,t)];
                        p = polyfit(x_extrap(1:2), y, 1);
