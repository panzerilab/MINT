...
if strcmp(func2str(corefunc), 'PID')
    ...
    if strcmp(corr, 'qe_shuffSub') || strcmp(corr, 'le_shuffSub')
        PID2_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, length(outputs));
        PID4_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, length(outputs));
        PID_shuff_corrected = repmat({zeros(opts.shuff,nTimepoints)}, 1, length(outputs));
        if opts.pid_constrained
            I1_2_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
            I1_4_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
            I2_2_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
            I2_4_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
            I12_2_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
            I12_4_shuff = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);

            I1_shuff_corrected = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
            I2_shuff_corrected = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
            I12_shuff_corrected = repmat({zeros(opts.shuff,nTimepoints)}, 1, 1);
        end
    end
            ...
            for np=2:length(npartition)
                for pidx = 1:npartition(np)
                    ...
                    if strcmp(corr, 'qe_shuffSub') || strcmp(corr, 'le_shuffSub')
                        ...
                        [~,~,PID_shuff_all] = feval(corefunc, inputs_p, outputs, naive_opts);
                        if opts.pid_constrained
                            [~,~,I1_shuff_all] = MI({inputs_p{1}, inputs_p{end}}, {'I(A;B)'}, naive_opts);
                            [~,~,I2_shuff_all] = MI({inputs_p{2}, inputs_p{end}}, {'I(A;B)'}, naive_opts);
                            [~,~,I12_shuff_all] = MI({cat(1, inputs_p{1}, inputs_p{2}), inputs_p{end}}, {'I(A;B)'}, naive_opts);
                        end

                        for shuffIdx = 1:opts.shuff
                            for outIdx = 1:length(outputs)
                                if npartition(np)==2
                                    PID2_shuff{outIdx}(shuffIdx,:) =  PID2_shuff{outIdx}(shuffIdx,:) + PID_shuff_all{outIdx}(shuffIdx,:)/2;
                                elseif npartition(np)==4
                                    PID4_shuff{outIdx}(shuffIdx,:) =  PID4_shuff{outIdx}(shuffIdx,:) + PID_shuff_all{outIdx}(shuffIdx,:)/4;
                                end
                            end
                            if opts.pid_constrained
                                if npartition(np)==2
                                    I1_2_shuff{1}(shuffIdx,:) =  I1_2_shuff{1}(shuffIdx,:) + I1_shuff_all{1}(shuffIdx,:)/2;
                                    I2_2_shuff{1}(shuffIdx,:) =  I2_2_shuff{1}(shuffIdx,:) + I2_shuff_all{1}(shuffIdx,:)/2;
                                    I12_2_shuff{1}(shuffIdx,:) =  I12_2_shuff{1}(shuffIdx,:) + I12_shuff_all{1}(shuffIdx,:)/2;
                                elseif npartition(np)==4
                                    I1_4_shuff{1}(shuffIdx,:) =  I1_4_shuff{1}(shuffIdx,:) + I1_shuff_all{1}(shuffIdx,:)/4;
                                    I2_4_shuff{1}(shuffIdx,:) =  I2_4_shuff{1}(shuffIdx,:) + I2_shuff_all{1}(shuffIdx,:)/4;
                                    I12_4_shuff{1}(shuffIdx,:) =  I12_4_shuff{1}(shuffIdx,:) + I12_shuff_all{1}(shuffIdx,:)/4;
                                end
                            end
                        end
                    end
            ...
            if strcmp(opts.bias,'qe') ||strcmp(opts.bias,'qe_shuffSub')
                ...
                if strcmp(opts.bias,'qe_shuffSub')
                    for t = 1:nTimepoints
                        for outIdx = 1:length(outputs)
                            for shuffIdx = 1:opts.shuff
                                y = [PID_shuff_all{outIdx}(shuffIdx,t),  PID2_shuff{outIdx}(shuffIdx,t),  PID4_shuff{outIdx}(shuffIdx,t)];
                                p = polyfit(x_extrap, y, 2);
                                PID_shuff_corrected{outIdx}(shuffIdx,t)  =  PID_shuff_corrected{outIdx}(shuffIdx,t) + p(3)/xtrp;
                            end
                        end
                        if opts.pid_constrained
                            for shuffIdx = 1:opts.shuff
                                y = [I1_shuff_all{1}(shuffIdx,t),  I1_2_shuff{1}(shuffIdx,t),  I1_4_shuff{1}(shuffIdx,t)];
                                p = polyfit(x_extrap, y, 2);
                                I1_shuff_corrected{1}(shuffIdx,t)  =  I1_shuff_corrected{1}(shuffIdx,t) + p(3)/xtrp;

                                y = [I2_shuff_all{1}(shuffIdx,t),  I2_2_shuff{1}(shuffIdx,t),  I2_4_shuff{1}(shuffIdx,t)];
                                p = polyfit(x_extrap, y, 2);
                                I2_shuff_corrected{1}(shuffIdx,t)  =  I2_shuff_corrected{1}(shuffIdx,t) + p(3)/xtrp;

                                y = [I12_shuff_all{1}(shuffIdx,t),  I12_2_shuff{1}(shuffIdx,t),  I12_4_shuff{1}(shuffIdx,t)];
                                p = polyfit(x_extrap, y, 2);
                                I12_shuff_corrected{1}(shuffIdx,t)  =  I12_shuff_corrected{1}(shuffIdx,t) + p(3)/xtrp;
                            end
                        end
                    end
                elseif strcmp(opts.bias,'le')||strcmp(opts.bias,'le_shuffSub')
                    ...
                    if strcmp(opts.bias,'le_shuffSub')
                        for t = 1:nTimepoints
                            for outIdx = 1:length(outputs)
                                for shuffIdx = 1:opts.shuff
                                    y = [PID_shuff_all{outIdx}(shuffIdx,t),  PID2_shuff{outIdx}(shuffIdx,t)];
                                    p = polyfit(x_extrap, y, 1);
                                    PID_shuff_corrected{outIdx}(shuffIdx,t)  =  PID_shuff_corrected{outIdx}(t) + p(2)/xtrp;
                                end
                            end
                        end
                        if opts.pid_constrained
                            for t = 1:nTimepoints
                                for shuffIdx = 1:opts.shuff
                                    y = [I1_shuff_all{1}(shuffIdx,t),  I1_2_shuff{1}(shuffIdx,t)];
                                    p = polyfit(x_extrap, y, 1);
                                    I1_shuff_corrected{1}(shuffIdx,t)  =  I1_shuff_corrected{1}(shuffIdx,t) + p(2)/xtrp;

                                    y = [I2_shuff_all{1}(shuffIdx,t),  I2_2_shuff{1}(shuffIdx,t)];
                                    p = polyfit(x_extrap, y, 1);
                                    I2_shuff_corrected{1}(shuffIdx,t)  =  I2_shuff_corrected{1}(shuffIdx,t) + p(2)/xtrp;

                                    y = [I12_shuff_all{1}(shuffIdx,t),  I12_2_shuff{1}(shuffIdx,t)];
                                    p = polyfit(x_extrap, y, 1);
                                    I12_shuff_corrected{1}(shuffIdx,t)  =  I12_shuff_corrected{1}(shuffIdx,t) + p(2)/xtrp;
                                end
                            end
                        end
                    end
    ...
    if strcmp(opts.bias,'qe_shuffSub') || strcmp(opts.bias,'le_shuffSub')
        for t = 1:nTimepoints
            ...
            if opts.pid_constrained
                ...
                I1_corrected(t) = I1_corrected(t) - mean(I1_shuff_corrected{1}(:,t));
                I2_corrected(t) = I2_corrected(t) - mean(I2_shuff_corrected{1}(:,t));
            end
        end
        if opts.pid_constrained
