...
for data = 1:length(AC_data)
    ...
    [uniqueValues, ~, indices] = unique([AC_data(data).stimulus, AC_data(data).choice], 'rows');
    indicesForCombinations = cell(size(uniqueValues, 1), 1);
    for uniqueIdx = 1:size(uniqueValues, 1)
        indicesForCombinations{uniqueIdx} = find(indices == uniqueIdx);
    end
    trainIndices_set = cell(size(indicesForCombinations,1), k_split);
    testIndices_set = cell(size(indicesForCombinations,1), k_split);
    for i = 1:size(indicesForCombinations)
        cvPartition = cvpartition(indicesForCombinations{i,1}, 'KFold', k_split);
        for fold = 1:k_split
            trainIdx = training(cvPartition, fold);
            testIdx = test(cvPartition, fold);
            trainIndices_set{i,fold} = indicesForCombinations{i,1}(trainIdx);
            testIndices_set{i,fold} = indicesForCombinations{i,1}(testIdx);
        end
    end    
    for fold = 1:1:cvPartition.NumTestSets
        field = ['Fold_', num2str(fold)];
        trainIndices_AC(data).(field) =  sort(vertcat(trainIndices_set{:, fold}));
        testIndices_AC(data).(field) =  sort(vertcat(testIndices_set{:, fold}));
    end
end
...
for data = 1:length(PPC_data)
    ...
    [uniqueValues, ~, indices] = unique([PPC_data(data).stimulus, PPC_data(data).choice], 'rows');
    indicesForCombinations = cell(size(uniqueValues, 1), 1);
    for uniqueIdx = 1:size(uniqueValues, 1)
        indicesForCombinations{uniqueIdx} = find(indices == uniqueIdx);
    end
    ...
    for fold = 1:1:cvPartition.NumTestSets
        field = ['Fold_', num2str(fold)];
        trainIndices_PPC(data).(field) =  sort(vertcat(trainIndices_set{:, fold}));
        testIndices_PPC(data).(field) =  sort(vertcat(testIndices_set{:, fold}));
    end
end


for data_idx = 1:length(PPC_data)    
     PPC_data(data_idx).trainIndices = trainIndices_PPC(data_idx);
     PPC_data(data_idx).testIndices = testIndices_PPC(data_idx);
     AC_data(data_idx).trainIndices = trainIndices_AC(data_idx);
     AC_data(data_idx).testIndices = testIndices_AC(data_idx);
end

save('AC_data.mat', 'AC_data');
save('PPC_data.mat', 'PPC_data');
