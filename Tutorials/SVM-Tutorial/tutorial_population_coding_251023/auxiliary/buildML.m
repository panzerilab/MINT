function [predictedLabels, labelsTest, dataTest, posteriorProbs, varargout] = buildML(data,labels,testSet,opts)
...
assert(ismember(opts.algorithm, {'linear_SVM', 'RBF_SVM', 'GLM'}), ...
    'Invalid algorithm specified: %s', opts.algorithm);
...
if SVM_flag                                                             %if SVM is chosen  
    ...
    if svm_family == 0
        svmModel = fitcsvm(dataTrain, labelsTrain, 'KernelFunction', 'linear', 'BoxConstraint', opts.hp_C, 'Standardize', false);        
    elseif svm_family == 2
        svmModel = fitcsvm(dataTrain, labelsTrain, 'KernelFunction', 'rbf', 'BoxConstraint',   opts.hp_C, 'KernelScale', (1/(2*opts.hp_gamma)), 'Standardize', false);
    end

    %'KernelScale', opts.hp_gamma, 
    warning('off', 'all');
...
for repIdx1 = 1:opts.optim_reps % loop over number of optimization parameters
    ...
    if svm_family == 0 % linear, just one hyperparam (C)
        for i = 1:setsCV % for each set of CV
            testIdx = find(cvPartition.test(i));                            
            trainIdx = find(cvPartition.training(i));
            svmModel = fitcsvm(dataTrain(trainIdx, :), labelsTrain(trainIdx), 'KernelFunction', 'linear', 'BoxConstraint', c_vals(repIdx1), 'Standardize', false);
            [optimPredictedLabels(testIdx), ~, ~, posteriorProbs] = predict(svmModel, dataTrain(testIdx, :), 'Binary', true);
        end
    ...
    elseif svm_family == 2 % RBF kernel, 2 hyperparameters (C and gamma)
        for repIdx2 = 1:opts.optim_reps % loop over gamma
            testIdx = find(cvPartition.test(i));                         
            trainIdx = find(cvPartition.training(i));
            svmModel = fitcsvm(dataTrain(trainIdx, :), labelsTrain(trainIdx), 'KernelFunction', 'rbf', 'BoxConstraint', c_vals(repIdx1), 'KernelScale', g_vals(repIdx2), 'Standardize', false);
            [optimPredictedLabels(testIdx), ~, ~, posteriorProbs] = predict(svmModel, dataTrain(testIdx, :), 'Binary', true);
        end
