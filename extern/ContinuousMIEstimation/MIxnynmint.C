...
void MIxnynmint(int dimx, int dimy, int K, int N, double *inputArray, double *MI) {
    int i, d;
    double **x;
    double *scal;
    double *min;
    ...
    // Copy the input data into the x array
    for (i = 0; i < N; i++) {
        for (d = 0; d < dimx + dimy; d++) {
            x[d][i] = inputArray[i * (dimx + dimy) + d];
        }
    }
    ...
    for (d = 0; d < dimx + dimy; d++) {
        me = s = 0;
        for (i = 0; i < N; i++) me += x[d][i];
        me /= N;
        for (i = 0; i < N; i++) s += (x[d][i] - me) * (x[d][i] - me);
        s /= (N - 1);
        s = sqrt(s);
        if (s == 0) {;}
        for (i = 0; i < N; i++) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    MIxnynmint(dimx, dimy, K, N, inputArray, MI);
}
