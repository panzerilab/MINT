classdef test_FIT < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testZerosInput(testCase)
            ...
            X = zeros(3, 100, 100);           
            Y = zeros(3, 100, 100);
            FIT_result = FIT(S, X, Y);
            assert(FIT_result == 0, 'Value should be zero.');
            warning('on', 'all');
        end
        ...
        function testNaNInput(testCase)
            ...
            Y = rand(3, 100, 100);           
            Y(2, 5) = NaN;
            testCase.verifyError(@()FIT(S, X, Y), 'sanitycheck:NaNInput');
        end
        ...
        function testRandiValues(testCase)
            ...
            btspflag = false;
            S = randi([1, 2], 1, 50);
            X = randi([1, 2], 1, 100, 50 );
            Y = randi([1, 2], 1, 100, 50);        
            FIT_result = FIT(S, X, Y);
            assert(FIT_result == 0);
            warning('on', 'all');
        end
        ...
        function testValidInput(testCase)
            ...
            FIT_result = FIT(S, X, Y, opts);
            assert((FIT_result - 0.5982) < 0.01);
        end
        ...
        function testInvalidBias(testCase)
            ...
            opts.n_binsY = 5;
            opts.bin_methodS = 'none';
            testCase.verifyError(@()FIT(S, X, Y, opts), 'FIT:UndefinedBiasCorrectionMethod');
        end
        ...
        function testInvalidBinning(testCase)
            ...
            opts.n_binsY = 5;
            opts.bin_methodS = 'none';
            testCase.verifyError(@()FIT(S, X, Y, opts), 'FIT:InvalidBinningMethod');
        end
    end
